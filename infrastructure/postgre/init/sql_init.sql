...
-- RAG: hierarchical chunks (parents + children) with hybrid search
CREATE TABLE IF NOT EXISTS rag_parents (
    id BIGINT NOT NULL PRIMARY KEY generated by default as identity,
    source_type VARCHAR(50) NOT NULL,   -- 'kb' | 'code' | 'article'
...
CREATE TABLE IF NOT EXISTS rag_chunks (
    id BIGINT NOT NULL PRIMARY KEY generated by default as identity,
    parent_id BIGINT NOT NULL REFERENCES rag_parents(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS rag_links (
    id BIGINT NOT NULL PRIMARY KEY generated by default as identity,
    parent_id BIGINT NOT NULL REFERENCES rag_parents(id) ON DELETE CASCADE,
    child_id BIGINT NOT NULL REFERENCES rag_chunks(id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS rag_parents_tsv_idx ON rag_parents USING GIN (content_tsv);
CREATE INDEX IF NOT EXISTS rag_chunks_tsv_idx ON rag_chunks USING GIN (content_tsv);
CREATE INDEX IF NOT EXISTS rag_chunks_vector_ivf ON rag_chunks USING ivfflat (vector vector_cosine_ops) WITH (lists = 100);
CREATE UNIQUE INDEX IF NOT EXISTS rag_links_parent_child ON rag_links (parent_id, child_id);
CREATE INDEX IF NOT EXISTS rag_links_child ON rag_links (child_id);
CREATE INDEX IF NOT EXISTS rag_metadata_source ON rag_metadata (source_type, source_path);
...
CREATE TABLE links (
   ...
   ingestion_status VARCHAR(20) NOT NULL DEFAULT 'queued',
...
CREATE INDEX links_ingestion_status ON links (ingestion_status);
