...
export class Analysis {
  ...
  /**
   * MQ feed analytics topic name.
   * @private
   * @type { string }
   */
  private readonly analytics_channel_name: string;
  ...
  constructor( service_name: string, mq_consumer: IMessageQueueSub, logger: ILogger, dbconn: IDatabase, key_store_pub: IKeyStorePub ) {
    ...
    this.analytics_channel_name = env.RSS_ANALYTICS_CHANNEL_NAME;
    ...
    // subscribe to RSS feed analytics channel, so we can update statistics for links amount per day, month and year
    ...
    this.mq_consumer.consume( [ this.logs_channel_name, this.analytics_channel_name ], self.update_stats.bind( this ) );
  }
  ...
  private async update_stats( { topic, message, trace_id } ): Promise<any> {
    if ( topic == this.analytics_channel_name ) {
      await this.update_ok_stats( { topic: topic, message: message, trace_id: trace_id } );
    } else if ( message.service == 'link_writer' && message.severity == LOG_SEVERITIES.LOG_SEVERITY_NOTICE ) {
  ...
  /**
   * Updates fetch times statistics when RSS feed fetch was successful.
   *
   * @param { Object } data This is the processed data received from Kafka consumer.
   *                        Object structure: topic, message, trace_id
   * @private
  ...
  private async update_ok_stats( { topic, message, trace_id } ): Promise<any> {
    ...
    const
      analysis_telemetry: Telemetry = ( trace_carrier !== null ? await new Telemetry( this.service_name, this.version, message.feed_url ).start( trace_carrier ) : null ),
      telemetry_name: string = 'analysis_update';

    try {
      if ( analysis_telemetry !== null ) {
        await analysis_telemetry.add_span( telemetry_name, { 'feed_url': message.feed_url }, 'Fetch successful' );
      }

      // no need to await for this log message, since we only debug-log it
      //this.logger.log_msg( 'writing stats data and updating fetch times for ' + message.feed_url, 0, LOG_SEVERITIES.LOG_SEVERITY_LOG );
      await this.dbconn.inc_fetch_times_with_ok_status( message.feed_url );
      ...
      this.logger.log_msg('Exception while trying to save statistical feed data of ' + message.feed_url + '\n' + err.message + '\ndata: ' + JSON.stringify( message ), 'ERR_ANALYSIS_FEED_FREQUENCY_UPDATE_FAILURE' );
      if ( analysis_telemetry !== null ) {
        await analysis_telemetry.add_span( telemetry_name, { 'feed_url': message.feed_url }, 'Exception while trying to save statistical feed data of ' + message.feed_url + '\n' + err.message + '\ndata: ' + JSON.stringify( message ), 1 );
        analysis_telemetry.close_active_span( telemetry_name );
      }
    }
